...
class NsharedpreferencesSerializer implements Serializer {
  ...
  NsharedpreferencesSerializer(LogInterceptor logInterceptor) {
    this.logInterceptor = logInterceptor;
  }

  @Override public <T> String serialize(String cipherText, T originalGivenValue) {
    NsharedpreferencesUtils.checkNullOrEmpty("Cipher text", cipherText);
    NsharedpreferencesUtils.checkNull("Value", originalGivenValue);
  ...
  @Override public DataInfo deserialize(String serializedText) {
    ...
    if (firstElement != null && firstElement.length() != 0) {
      try {
        ...
        logInterceptor.onLog("NsharedpreferencesSerializer -> " + e.getMessage());
      }
    }
